<script>
  ...
  async function createDiagnosisReport(currentTime) {
    const pdfBytes2 = await createPDF2(
      newMedication,
      params.hospitalName,
      get(doctorInfo).doctorName,
      get(doctorHospital).hospitalLogo,
  ...
  async function handleSubmit() {
    ...
    showModal = false;
    currentStep = 0;
    window.location.href = `#/doctorhome/hospitals`;
  }
...
<div class="p-6 bg-gray-100 min-h-screen">
  ...
  <div class="mt-6 bg-white p-6 rounded-md shadow-xl relative">
    ...
    {#if showModal}
      ...
      >
        ...
        >
          <button
            class="absolute top-2 right-2 text-2xl font-bold hover:scale-110"
            on:click={cancelkoro}
            >&times;
          </button>
  ...
  <!-- <button class="btn btn-accent my-10" on:click={handleDownload}
    >Print PDF</button
  > -->
</div>
