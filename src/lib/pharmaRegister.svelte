<script>
  ...
  async function uploadImageAndLogo(id, image, logo) {
    let { data: res1 } = await supabase.storage
      .from("companyImage")
      .upload(id, image, {
        cacheControl: "0",
    ...
    // let { data: link1 } = await supabase.storage
    //   .from("companyImage")
    //   .getPublicUrl(id);
    ...
    // let { data: link2 } = await supabase.storage
    //   .from("companyLogo")
    //   .getPublicUrl(id);

    let payload = {
      id: id,
      urlPhoto: `${bucketUrl}/companyImage/${id}`,
      urlLogo: `${bucketUrl}/companyLogo/${id}`,
    };

    fetch(serverUrl + "company/change-image-logo", {
      method: "POST",
      body: JSON.stringify(payload),
    });
  }
  ...
  async function handleSubmit(event) {
    ...
    await fetch(serverUrl + "company/register", {
      method: "POST",
      body: formData,
    })
      .then((response) => {
...
>
  ...
  <div bind:this={modal} class="fixed z-10 inset-0 overflow-y-auto hidden">
    <div class="flex items-end justify-center min-h-screen">
      ...
      >
        <!-- Modal Header -->
        <div class="bg-green-500 text-white px-6 py-4 mb-4 rounded-t-lg">
        ...
        <div class="mb-4 px-6 py-2">
          <p class="text-gray-700 mb-4">
            Congratulations on successfully signing up. Below is your Company
            ID:
          </p>
        ...
        <div class="flex gap-4 px-6 py-4">
          <button on:click={copyToClipboard} class="btn btn-outline btn-primary"
            >Copy</button
          >
