<script>
  ...
  function confirmSave() {
    const index = drugList.findIndex((p) => p.id === expandedProductId);
    drugList[index] = { ...tempProduct };
    showConfirmationModal = false;
    expandedProductId = null;
    

    updateDrug(tempProduct);
    tempProduct = null;
  }

  async function updateDrug(updatedDrug){
    await fetch(serverUrl + "drug/update-drug", {
      method: "POST",
  ...
  function toggleDetails(id) {
    if (expandedProductId === id) {
      ...
      const product = drugList.find((p) => p.id === id);
  ...
  async function deleteProduct(productId) {
    ...
    drugList = drugList.filter((product) => product.id !== productId);

    let payload = {id : productId}
    await fetch(serverUrl + "drug/remove-drug", {
      method: "POST",
  ...
  function applyFilters() {
    filteredProducts = drugList;
    filteredProducts = drugList.filter((product) => {
      ...
      let matchesVariant =
        !filterVariant ||
        product.variant.toLowerCase().includes(filterVariant.toLowerCase());
      ...
      let matchesLowerSale =
        filterLowerSale == null || product.unitSold >= filterLowerSale;
      ...
      let matchesUpperSale =
        ...
        product.unitSold <= filterUpperSale;
  ...
  async function getDrugList() {
    let payload = { company: get(companyInfo).companyInfo["name"] };
    await fetch(serverUrl + "drug/get-drug-list", {
      method: "POST",
      body: JSON.stringify(payload),
    })
      .then((response) => {
          ...
          <section class="mt-6 w-full">
            <div class="stats mx-4 shadow">
              ...
              <div class="stat">
                ...
                <div class="stat-desc">Prescribed & Trusted</div>
              </div>
                  ...
                  <div class="bg-white p-4 rounded shadow">
                    ...
                    >
                      <button
                        ...
                        on:click={() => toggleDetailsView(drug["id"])}
                        >View Details</button
                      >
                      <button
                        class="btn bg-lime-400 hover:bg-lime-500"
                        on:click={() => toggleDetails(drug["id"])}
                      >
                        <img
                    ...
                    {#if expandedViewProductId}
                      ...
                      >
                        <div class="modal modal-open">
                          <div class="modal-box custom-modal-width relative">
                            <!-- relative positioning for the modal-box -->
                            <!-- Product Image - Floating to the upper right corner with a frame -->
                            {#each drugList.filter((p) => p["id"] === expandedViewProductId) as drug}
                              <img
                                src={drug["photo"]}
                                alt={drug["name"]}
                        ...
                        <div class="bg-white p-6 rounded-md shadow-lg">
                          ...
                          <div class="mt-4 flex justify-end space-x-4">
                            ...
                            <button
                              ...
                              on:click={() => {
                                deleteProduct(confirmedDeleteId);
                          ...
                          <div class="modal-box">
                            <form on:submit|preventDefault={saveChanges}>
                              ...
                              <div class="flex items-center">
                                ...
                                <img
                                  src={images[tempProduct.variant]}
                      ...
                      >
                        <div class="bg-white p-6 rounded-md shadow-lg">
                          ...
                          <div class="modal-action mt-4">
                            <button
                              class="btn btn-secondary"
                              on:click={cancelSave}>Cancel</button
                            >
                            <button
                              class="btn btn-danger"
                              on:click={confirmSave}>Confirm</button
                            >
                          </div>
                        </div>
                      </div>
                ...
                {#each filteredProducts as product (product.id)}
                  ...
                  <div class="bg-white p-4 rounded shadow">
                    ...
                    >
                      <button
                        class="mr-2 btn btn-primary"
                        on:click={() => toggleDetailsView(product.id)}
                        >View Details</button
                      >
                      <button
                        class="btn bg-lime-400 hover:bg-lime-500"
                        on:click={() => toggleDetails(product.id)}
                      >
                        <img
                          src="https://aaitclybvvendvuswytq.supabase.co/storage/v1/object/public/BDeHR/pencil-edit-button-svgrepo-com.svg"
                          class="transform transition duration-300 hover:rotate-12 mr-2"
