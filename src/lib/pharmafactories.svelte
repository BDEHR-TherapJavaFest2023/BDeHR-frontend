<script>
  ...
  onMount(async () => {
    ...
    map = L.map("map").setView(
      [get(companyInfo).companyInfo["latitude"], get(companyInfo).companyInfo["longitude"]],
      14
    );
    L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
      attribution: "&copy; OpenStreetMap contributors",
    }).addTo(map);
    L.marker([get(companyInfo).companyInfo["latitude"], get(companyInfo).companyInfo["longitude"]]).addTo(
      map
    );
  ...
  <div class="flex">
    ...
    <div class="ml-64 w-full">
      ...
      <div class="bg-white rounded-lg shadow-md p-6 mb-8 mx-8">
        <h2 class="text-2xl font-extrabold text-blue-600 mb-4">About Us</h2>
        <p>{get(companyInfo).companyInfo["factoryAbout"]}</p>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mx-8">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
          ...
          <p>{get(companyInfo).companyInfo["factoryAddress"]}</p>
        </div>

        <!-- Contact Information -->
        <div class="bg-white rounded-lg shadow-md p-6">
          <h2 class="text-xl font-bold text-blue-600 mb-4">Contact Us</h2>
          <div class="flex items-center space-x-2 mb-2">
            ...
            <p>
              <strong>Phone: </strong>
              {get(companyInfo).companyInfo["phone"]}
            </p>
          </div>
          <div class="flex items-center space-x-2 mb-2">
            <img
            ...
            <p>
              <strong>Email: </strong>
              {get(companyInfo).companyInfo["email"]}
            </p>
          </div>
        </div>
      </div>
